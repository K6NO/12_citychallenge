...
// returns the name of the day. index 0 is today
function getDayName (index) {
    var days = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
    let dat = new Date();
    return days[(dat.getDay() + index)%days.length];
}
...
var saveAndCheckWaitListForMatch = function(req, res, next) {
    ...
    if(req.session.passport) {
        ...
        req.body.steps.forEach(function (step) {
            ...
            newCurrentChallenge.steps.push(newStep);
            newStep.save(function (err) {
                if (err) return next(err);
            })
        });

        for (let i=0; i<req.body.time; i++) {
            let step = new Step({
                "description" : getDayName(i)
            });
            newCurrentChallenge.progressBar.push(step);
                                ...
                                CurrentChallenge.findOneAndUpdate(
                                    ...
                                    .exec(function (err, firstChallenge) {
                                        ...
                                        CurrentChallenge.findOneAndUpdate(
                                            ...
                                            {new: true})
                                            .populate('user challenge partner partnerChallenge progressBar')
                                            .exec(function (err, secondChallenge) {
                                                ...
                                                // return the user's currentChallenge
                                                req.saved = true;
